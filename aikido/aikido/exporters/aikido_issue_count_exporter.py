...
class AikidoIssueCountExporter:
    ...
    async def export(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            response = await self.client.get("/issue-counts")
            logger.info(f"[{self.__class__.__name__}] kind={self.KIND} API status={response.status_code}")
            response.raise_for_status()
            
            issue_counts = response.json()
            logger.info(f"[{self.__class__.__name__}] kind={self.KIND} Received {len(issue_counts)} items from API")
            ...
            for count in issue_counts:
                transformed_count = {
                    "issue_groups": count.get("issue_groups", {}),
                    "issues": count.get("issues", {})
                }
                ...
                transformed_counts.append(transformed_count)
            logger.info(f"[{self.__class__.__name__}] kind={self.KIND} Successfully exported {len(transformed_counts)} issue counts")
