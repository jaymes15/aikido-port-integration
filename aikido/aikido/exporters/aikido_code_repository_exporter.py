...
class AikidoCodeRepositoryExporter:
    ...
    async def export(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"[{self.__class__.__name__}] kind={self.KIND} Received {len(code_repositories)} items from API")
            ...
            for repo in code_repositories:
                ...
                logger.debug(f"[{self.__class__.__name__}] kind={self.KIND} Exported repo | id={transformed_repo['id']} name={transformed_repo['name']} provider={transformed_repo['provider']}")
                transformed_repos.append(transformed_repo)
            logger.info(f"[{self.__class__.__name__}] kind={self.KIND} Successfully exported {len(transformed_repos)} code repositories")
            return transformed_repos
            
        except Exception as e:
            logger.error(f"[{self.__class__.__name__}] kind={self.KIND} Failed to export code repositories | error={e}", exc_info=True)
            return []
