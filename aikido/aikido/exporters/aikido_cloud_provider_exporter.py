...
class AikidoCloudProviderExporter:
    ...
    async def export(self) -> List[Dict[str, Any]]:
        try:
            ...
            logger.info(f"[{self.__class__.__name__}] kind={self.KIND} Received {len(cloud_providers)} items from API")
            ...
            for provider in cloud_providers:
                ...
                logger.debug(f"[{self.__class__.__name__}] kind={self.KIND} Exported provider | id={transformed_provider['id']} name={transformed_provider['name']} environment={transformed_provider['environment']}")
                transformed_providers.append(transformed_provider)
            logger.info(f"[{self.__class__.__name__}] kind={self.KIND} Successfully exported {len(transformed_providers)} cloud providers")
            return transformed_providers
        except Exception as e:
            logger.error(f"[{self.__class__.__name__}] kind={self.KIND} Failed to export cloud providers | error={e}", exc_info=True)
            return []
