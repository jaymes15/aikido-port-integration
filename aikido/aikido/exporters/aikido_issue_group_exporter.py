...
class AikidoIssueGroupExporter:
    ...
    async def export(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            response = await self.client.get("/issue-groups")
            logger.info(f"[{self.__class__.__name__}] kind={self.KIND} API status={response.status_code}")
            response.raise_for_status()
            
            issue_groups = response.json()
            logger.info(f"[{self.__class__.__name__}] kind={self.KIND} Received {len(issue_groups)} items from API")
            ...
            for group in issue_groups:
                ...
                logger.debug(f"[{self.__class__.__name__}] kind={self.KIND} Exported group | id={transformed_group['id']} title={transformed_group['title']} severity={transformed_group['severity']}")
                transformed_groups.append(transformed_group)
            logger.info(f"[{self.__class__.__name__}] kind={self.KIND} Successfully exported {len(transformed_groups)} issue groups")
            return transformed_groups
            
        except Exception as e:
            logger.error(f"[{self.__class__.__name__}] kind={self.KIND} Failed to export issue groups | error={e}", exc_info=True)
            return []
