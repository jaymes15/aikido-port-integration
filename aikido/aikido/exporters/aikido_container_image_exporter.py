...
class AikidoContainerImageExporter:
    ...
    async def export(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"[{self.__class__.__name__}] kind={self.KIND} Received {len(container_images)} items from API")
            ...
            for image in container_images:
                ...
                logger.debug(f"[{self.__class__.__name__}] kind={self.KIND} Exported image | id={transformed_image['id']} name={transformed_image['name']} tag={transformed_image['tag']}")
                transformed_images.append(transformed_image)
            logger.info(f"[{self.__class__.__name__}] kind={self.KIND} Successfully exported {len(transformed_images)} container images")
            return transformed_images
            
        except Exception as e:
            logger.error(f"[{self.__class__.__name__}] kind={self.KIND} Failed to export container images | error={e}", exc_info=True)
